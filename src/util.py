...
def crop_aviris_scene(folder_path: str, section_size=(256, 256)):
    """
    Crop a full AVIRIS HSI scene into smaller sections sequentially.
    Only valid sections (no black/constant areas) are saved.
    """
    # ===== Load full scene =====
    ...
    for i in range(n_vert):
        ...
        for j in range(n_horiz):
            ...
            # VALIDATION: Ensure section is not a "black area"
            # If max == min, the patch is constant (usually zeros or dead pixels)
            if np.max(section_hsi.data) <= np.min(section_hsi.data):
                continue

            valid_count += 1
            # Deterministic naming based on the valid sequence
            section_name = f"{scene_name}_s{valid_count}" 
    ...
    print(f"Total checked: {total_checked} | Saved {valid_count} valid sections.")

def split_aviris_sections(folder_path: str, train_ratio=0.8, seed=42):
    """
    Distributes pre-validated sequential sections into train/test subfolders[cite: 4].
    """
    sections_dir = os.path.join(folder_path, "sections")
    train_dir = os.path.join(sections_dir, "train")
    test_dir = os.path.join(sections_dir, "test")
    
    os.makedirs(train_dir, exist_ok=True)
    os.makedirs(test_dir, exist_ok=True)
    
    # Identify all .npy valid patches in the root sections folder[cite: 4]
    all_sections = [f for f in os.listdir(sections_dir) if f.endswith('.npy') and os.path.isfile(os.path.join(sections_dir, f))]
    
    # Sort for deterministic order before shuffling[cite: 4]
    all_sections.sort()
    
    random.seed(seed)
    random.shuffle(all_sections)
    
    n_train = int(len(all_sections) * train_ratio)
    
    for idx, filename in enumerate(all_sections):
        src = os.path.join(sections_dir, filename)
        target_subfolder = train_dir if idx < n_train else test_dir
        dst = os.path.join(target_subfolder, filename)
        
        shutil.move(src, dst)

    print(f"Distributed {len(all_sections)} valid patches: {n_train} Train, {len(all_sections)-n_train} Test.")
